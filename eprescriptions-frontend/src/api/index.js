...
api.interceptors.request.use((config) => {
  ...
  if (user?.llave_privada && config.attachPrivKeys) {
    ...
    config.headers["X-Priv-Keys"] = btoa(
      unescape(encodeURIComponent(user.llave_privada)),
    );
  }
  return config;
});
...
api.interceptors.response.use(
  ...
  (err) => {
    ...
    if (status === 401 && /falta cabecera x-priv-keys/i.test(message || "")) {
      ...
      err.uiMessage =
        "Para acceder necesitas iniciar sesión con tus llaves privadas.";
      return Promise.reject(err);
    }

    if (
      status === 401 &&
      /sesión expirada|token inv[áa]lido/i.test(message || "")
    ) {
      useAuthStore.getState().logout();
    }
...
function translateError(status, message) {
  ...
  if (status === 403) {
    if (/llave\s+(privada|ec|rsa)\s+no\s+pertenece/i.test(message)) {
      ...
      if (/llave\s+rsa\s+no\s+pertenece/i.test(message))
        return "Tu llave RSA no coincide con la cuenta · revisa el archivo .pem que subiste";
  ...
  if (status === 400) {
    if (/INTEGRIDAD/i.test(message))
      return "INTEGRIDAD COMPROMETIDA: el hash SHA3-256 o el TAG GCM no coincide";
    if (/FIRMA/i.test(message))
      return "FIRMA INVÁLIDA: la firma ECDSA no pudo verificarse";
...
export const usuariosAPI = {
  ...
  verificarEmail: (token) =>
    api.get("/usuarios/verificar-email", { params: { token } }),
  reenviarVerificacion: (email, username) =>
    api.post("/usuarios/reenviar-verificacion", { email, username }),
...
export const adminAPI = {
  solicitudes: (estado = "pendiente") =>
    api.get(`/admin/solicitudes?estado=${encodeURIComponent(estado)}`),
  aprobar: (id) => api.post(`/admin/solicitudes/${id}/aprobar`),
  suspender: (id, datos) =>
    api.post(`/admin/solicitudes/${id}/suspender`, datos),
...
export const dispensacionTicketsAPI = {
  ...
  firmarPaciente: (eventoId, datos) =>
    api.post(
      `/recetas/eventos-dispensacion/${eventoId}/firmar-paciente`,
      datos,
    ),
...
export const recetasAPI = {
  ...
  porPaciente: (pacienteId) =>
    api.get(`/recetas/paciente/${pacienteId}`, withKeys),
  porMedico: (medicoId) => api.get(`/recetas/medico/${medicoId}`),
  porFarmaceutico: (farmId) =>
    api.get(`/recetas/farmaceutico/${farmId}`, withKeys),
  ...
  cancelar: (recetaId, datos) =>
    api.post(`/recetas/${recetaId}/cancelar`, datos),
  nuevaVersion: (recetaId, datos) =>
    api.post(`/recetas/${recetaId}/nueva-version`, datos),
  verificarFirmas: (recetaId) =>
    api.get(`/recetas/${recetaId}/verificar-firmas`),
  porId: (recetaId) => api.get(`/recetas/${recetaId}`, withKeys), //Para el buscador de recetas por id
  dispensablesPaciente: (pacienteId) =>
