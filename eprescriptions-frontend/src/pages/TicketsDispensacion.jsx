...
export default function TicketsDispensacion() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const fetcher =
          role === "paciente"
            ? recetasAPI.porPaciente(user.id)
            : role === "medico"
              ? recetasAPI.porMedico(user.id)
              : role === "farmaceutico"
                ? recetasAPI.porFarmaceutico(user.id)
        ...
        if (!cancelled)
          setErrorRec(err?.uiMessage || "No se pudieron cargar las recetas");
      } finally {
  ...
  const headerTitle =
    role === "paciente"
      ? "Acuses de dispensación"
      : role === "medico"
        ? "Dispensaciones de tus recetas"
        : "Tus dispensaciones";

  const headerSubtitle =
    role === "paciente"
      ? "Selecciona una receta para ver sus dispensaciones y firmar el acuse de las que falten."
      : role === "medico"
        ? "Selecciona una receta para ver el detalle de cada dispensación."
        : "Selecciona una receta para ver las dispensaciones que realizaste.";
  ...
  return (
    <PageTransition>
      <div className="space-y-6">
        ...
        {selected && <Breadcrumb receta={selected} onRoot={back} />}
  ...
  return (
    <nav className="flex items-center gap-2 text-xs text-[color:var(--text-secondary)] flex-wrap">
      ...
      <span className="font-semibold text-[color:var(--blue-deep)]">
        Receta #{receta.id}
        {receta.medicamento && receta.medicamento !== "(cifrado)"
          ? ` · ${receta.medicamento}`
          : ""}
      </span>
    </nav>
  ...
  if (recetas.length === 0) {
    return (
      <EmptyState
        title={
          role === "paciente"
            ? "Sin recetas a tu nombre"
            : role === "medico"
              ? "Sin recetas emitidas"
              : "Sin recetas dispensadas"
        }
        message={
          role === "paciente"
            ? "Cuando un médico emita una receta y la farmacia la dispense, aparecerá aquí."
            : role === "medico"
        ...
        return (
          ...
          >
            <div className="flex items-center gap-3 p-4">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  <div className="font-heading text-[15px] truncate">
                    {r.medicamento && r.medicamento !== "(cifrado)"
                      ? r.medicamento
                      : `Receta #${r.id}`}
                  </div>
                  ...
                  {pendCount > 0 && (
                    <span
                      ...
                      style={{
                        ...
                        border: "1px solid rgba(224,135,0,0.40)",
                        color: "#9A6700",
                      }}
                    >
                      <Hourglass size={10} /> {pendCount} acuse
                      {pendCount > 1 ? "s" : ""} por firmar
                    </span>
                  )}
                </div>
                <div className="text-[11px] text-[color:var(--text-secondary)] flex flex-wrap gap-x-3 gap-y-0.5 mt-1">
                  ...
                  {r.paciente_username && role !== "paciente" && (
                    <span>pac @{r.paciente_username}</span>
                  )}
                  <span
                    className={
                      realizadas === 0
                        ? ""
                        : "font-semibold text-[color:var(--blue-deep)]"
                    }
                  >
              ...
              <ChevronRight
                size={16}
                className="shrink-0 text-[color:var(--text-secondary)] group-hover:translate-x-1 transition-transform"
              />
            </div>
          </motion.button>
        );
    ...
    <SecureCard className="flex flex-col md:flex-row md:items-center gap-4 justify-between">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            <div className="font-heading text-lg">
              Dispensación #{ev.numero_dispensacion}
            </div>
            <span
          ...
          <div className="text-xs text-[color:var(--text-secondary)] flex flex-wrap gap-x-3 gap-y-1 mt-1.5">
            <span className="flex items-center gap-1">
              <Stamp size={11} /> farm @
              {ev.farmaceutico_username || ev.farmaceutico_id}
            </span>
            {ev.medico_username && (
              <span className="flex items-center gap-1">
                <Stethoscope size={11} /> dr.@{ev.medico_username}
              </span>
            )}
            {ev.paciente_username && (
              <span className="flex items-center gap-1">
                <User size={11} /> pac @{ev.paciente_username}
              </span>
            )}
            <span className="flex items-center gap-1">
              <Calendar size={11} /> {formatDate(ev.timestamp)}
            </span>
            {ev.fecha_firma_paciente && (
              <span className="flex items-center gap-1 text-[color:var(--emerald)]">
                <Check size={11} /> firmado{" "}
                {formatDate(ev.fecha_firma_paciente)}
              </span>
            )}
          </div>
      ...
      <div className="shrink-0 flex items-center gap-2">
        ...
        <motion.button
          ...
          className="btn btn-ghost btn-sm"
        >
          <FileSignature size={14} /> Ver detalle
        </motion.button>
      </div>
    </SecureCard>
  );
...
function DetalleEventoModal({ evento, receta, onClose }) {
  return (
    <Modal
      open={!!evento}
      onClose={onClose}
      title={
        evento
          ? `Dispensación #${evento.numero_dispensacion} · Receta #${evento.receta_id}`
          : ""
      }
      wide
    >
      {evento && (
        <div className="space-y-4">
            ...
            >
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                <Info
                  label="Medicamento"
                  value={
                    receta.medicamento !== "(cifrado)"
                      ? receta.medicamento
                      : `Receta #${receta.id}`
                  }
                />
                {receta.dosis && receta.dosis !== "(cifrado)" && (
                  <Info label="Dosis" value={receta.dosis} />
                )}
                {receta.cantidad > 0 && (
                  <Info label="Cantidad" value={`x${receta.cantidad}`} />
                )}
                <Info
                  label="Paciente"
                  value={`@${receta.paciente_username || receta.paciente_id}`}
                />
                <Info
                  label="Médico"
                  value={`dr.@${receta.medico_username || receta.medico_id}`}
                />
                <Info
                  label="Estado"
                  value={<StatusChip estado={receta.estado} />}
                />
              </div>
              ...
              <div className="mt-3 pt-3 border-t border-[var(--border-subtle)]">
                <div className="label-xs mb-1">Dispensaciones</div>
                <div className="flex items-end gap-1.5">
                  ...
                  <span className="text-sm text-[color:var(--text-secondary)] mb-0.5">
                    / {receta.dispensaciones_permitidas}
                  </span>
                </div>
                <div
                ...
                >
                  <motion.div
                    ...
                    initial={{ width: 0 }}
                    animate={{
                      width: `${(receta.dispensaciones_realizadas / receta.dispensaciones_permitidas) * 100}%`,
                    }}
          ...
          >
            <div className="label-xs mb-3 flex items-center gap-1.5">
              <Stamp size={11} /> Evento de dispensación
            </div>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              <Info
                label="Farmacéutico"
                value={`@${evento.farmaceutico_username || evento.farmaceutico_id}`}
              />
              <Info label="Fecha" value={formatDate(evento.timestamp)} />
              <Info
                label="Acuse paciente"
                value={
                  evento.fecha_firma_paciente
                    ? formatDate(evento.fecha_firma_paciente)
                    : "—"
                }
              />
            </div>
            ...
            {/* Firmas */}
            <div className="mt-3 pt-3 border-t border-[var(--border-subtle)]">
              <div className="label-xs mb-2">Estado de firmas</div>
              <div className="flex items-center gap-4">
                <FirmaDot
                  label="Farmacéutico"
                  ok={!!evento.firma_farmaceutico}
                />
                <FirmaDot label="Paciente" ok={!!evento.firma_paciente} />
              </div>
            </div>
          </div>
...
function SignTicketModal({ ticket, onClose, onDone }) {
  ...
  const submit = async () => {
    ...
    setBusy(true);
    try {
      await dispensacionTicketsAPI.firmarPaciente(ticket.id, {
        llave_privada: key,
      });
      toast.success(
        `Acuse firmado · dispensación #${ticket.numero_dispensacion} confirmada`,
      );
      onDone();
    } catch (err) {
  ...
  return (
    <Modal
      ...
      title={
        ticket
          ? `Firmar acuse de dispensación #${ticket.numero_dispensacion}`
          : ""
      }
    >
      {ticket && (
        <div className="space-y-4">
          ...
          >
            La farmacia <strong>@{ticket.farmaceutico_username}</strong> firmó
            la dispensación
            <strong> #{ticket.numero_dispensacion}</strong> de la receta{" "}
            <strong>#{ticket.receta_id}</strong> el{" "}
            {formatDate(ticket.timestamp)}. Tu firma sobre el mismo manifiesto
            canónico confirma que recibiste el medicamento (acuse
            no-repudiable).
          </div>
          ...
          <div className="flex justify-end gap-2 pt-2 border-t border-[var(--border-subtle)]">
            ...
            >
              <ShieldCheck size={14} />{" "}
              {busy ? "Firmando…" : "Firmar con ECDSA P-256 + SHA3-256"}
            </button>
          </div>
        </div>
      )}
