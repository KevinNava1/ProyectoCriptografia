    ...
    const load = async () => {
      ...
      try {
        ...
        if (user.rol === "paciente") {
          ...
          //Quite la parte que muestra las recetas recien emitidas al farmaceutico, solo vera en el dash lo que el ha dispensado
  ...
  return (
    <PageTransition>
      ...
      >
        ...
        >
          <div>
            ...
            <p className="text-[color:var(--text-secondary)] mt-2 text-sm max-w-xl">
              Resumen en tiempo real de tus recetas y el estado criptográfico
              del sistema.
            </p>
          </div>
        ...
        {/* Poner de nuevo en col 4 y descomentar CryptoStatus para regresar el card*/}
        <motion.section
          variants={item}
        ...
        >
          ...
          />
          <Kpi {...kpi3} />
          {/*<CryptoStatus /> Descomentar para que reaparezca el card de crypto status*/}
        </motion.section>
...
function Kpi({ label, value, icon: Icon, accent }) {
  return (
    <SecureCard className="relative overflow-hidden min-h-[124px] flex flex-col justify-between p-4 sm:p-5">
      ...
      <div
        className="font-heading text-3xl sm:text-4xl mt-2"
        style={{ color: accent }}
      >
  ...
  return (
    <SecureCard className="relative overflow-hidden min-h-[124px] flex flex-col justify-between p-4 sm:p-5">
      <div
        ...
        style={{
          background:
            "radial-gradient(circle at 30% 30%, #00A870, transparent 60%)",
        }}
      />
      ...
      <div className="relative">
        ...
        <div className="text-[10px] text-[color:var(--text-secondary)] mt-1 tracking-wide">
          AES-128-GCM · ECDSA P-256 + SHA3-256
        </div>
      </div>
    </SecureCard>
  );
    ...
    return (
      <Link to="/nueva-receta">
        <motion.span
          whileHover={{
            scale: 1.04,
            boxShadow: "0 0 24px rgba(10,132,255,0.45)",
          }}
          whileTap={{ scale: 0.97 }}
