...
import { Pill, User, Calendar, Stethoscope, ShieldCheck, ShieldAlert, Hash, RotateCw, Download, Copy, Check } from 'lucide-react'
...
export default function RxTemplate({ receta, dimmed = false, flipped: flippedProp, onFlipChange }) {
    ...
    >
      ...
      >
        ...
        >
          ...
          <section className="relative px-5 sm:px-6 pt-4 z-10 flex-1" style={{ minHeight: 0, overflow: 'hidden' }}>
            ...
            <div className="flex items-baseline gap-3 flex-wrap mt-3">
              ...
              <span className="text-sm text-[color:var(--text-secondary)]">
                · <strong style={{ color: 'var(--text-primary)' }}>{tampered ? '—' : receta.cantidad}</strong> unidades
              </span>
              <span className="text-sm text-[color:var(--text-secondary)]">
                · <strong style={{ color: 'var(--text-primary)' }}>{receta.dispensaciones_permitidas ?? 1}</strong> dispensaciones
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mt-5">
              <Meta icon={User} label="Paciente" value={`@${receta.paciente_username || `id${receta.paciente_id}`}`} />
          ...
          <div className="absolute pointer-events-none capsule-float z-0" style={{ top: 18, right: 28, opacity: 0.18 }}>
        ...
        >
          ...
          <header className="relative px-5 sm:px-6 pt-5 pb-3 flex items-center gap-3 z-10">
            ...
            <button type="button" onClick={() => setFlipped(false)} className="btn btn-ghost btn-sm">
...
function TamperedBanner({ motivo }) {
  return (
    <div
      ...
      style={{ background: 'rgba(180,35,24,0.08)', border: '1px solid rgba(180,35,24,0.40)' }}
    >
      ...
      <div className="min-w-0">
        ...
        <div className="text-[11px] mt-0.5" style={{ color: '#7A1F12' }}>
          {motivo || 'La firma ECDSA del médico no verifica. No consumas el medicamento; contacta a tu médico.'}
        </div>
      </div>
    </div>
  )
      ...
      >
        ...
        <AnimatePresence>
          {has && (
            ...
            >
              <button type="button" onClick={download} className="inline-flex items-center gap-1 text-[10.5px] px-2 py-1 rounded hover:bg-white/40 transition-colors" style={{ color: palette.fg }}>
                <Download size={11} /> Descargar
              </button>
              <button type="button" onClick={copy} className="inline-flex items-center gap-1 text-[10.5px] px-2 py-1 rounded hover:bg-white/40 transition-colors" style={{ color: palette.fg }}>
